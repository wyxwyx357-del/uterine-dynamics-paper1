...
The task generator creates the annotation template with two blinded annotator IDs and the exact frame/point roles. `algorithm_results_template.csv` is optional and must be populated from an independent task-level export, without clinical labels or tracking-coordinate columns.
